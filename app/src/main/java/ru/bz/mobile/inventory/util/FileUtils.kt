...
object FileUtils {
            ...
            stream.readLines().asSequence().drop(1).filter { line -> line.isNotEmpty() }
                .map { it.split(delimiter) }.map { list ->
                    IOP.Dto(
                        ...
                        clot = list[4],
                        item = list[5],
                        qstr = list[6].trim().replace(',', '.').toDouble(),
                        unit = list[7],
                        porn = list[8],
                    )
                }.toList()
    ...
    fun exportCsv(
        ...
        outputStream.bufferedWriter().use { out ->
            out.write("orno${delimiter}pono${delimiter}cwar${delimiter}loca${delimiter}clot${delimiter}item${delimiter}qnty${delimiter}date${delimiter}stkr${delimiter}lock${delimiter}ilqd${delimiter}slca\n")
